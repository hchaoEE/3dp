...
import { DefLayer } from './DefLayer';
import { BondingLayer } from './BondingLayer';

export function FpEditor3D() {
  const { dies, modules, floorplan, tsvPlan, hbPlan, violations, defData } = useFpStore();
  ...
  return (
    <div className="relative w-full h-full bg-gray-950">
      ...
      >
        ...
        {dies.map((die, i) => {
          const dieDefData = defData[die.id];
          if (!dieDefData) return null;
          return (
            <DefLayer
              key={`def-${die.id}`}
              defData={dieDefData}
        ...
        {/* Bonding Layer from DEF */}
        {dies.map((die) => {
          const dieDefData = defData[die.id];
          if (!dieDefData || dieDefData.bondingPins.length === 0) return null;
          return (
            <BondingLayer
              key={`bonding-${die.id}`}
              defData={dieDefData}
