...
# reset, instr, valid, valid_reg, memwrite, suspend, ready, pc (core_without_dmem)
set core_ctrl_list [concat \
  [match_pins reset*] \
  [match_pins instr*] \
  [match_pins valid*] \
  [match_pins memwrite*] \
...
if {[llength $core_ctrl_list]} {
  set_io_pin_constraint -region "up:{20 20 1180 400}" -pin_names [concat {*}$core_ctrl_list]
}
